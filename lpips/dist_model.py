...
class DistModel(BaseModel):
    ...
    def forward_var(self, in0, in1):
        ''' Pretty much the same as this.forward, but in0 and in1 are Variable
            objects so we can collect gradients
        '''
        assert min([torch.min(in0).data[0], torch.min(in1).data[0]]) >= -1.0
        assert max([torch.max(in0).data[0], torch.max(in1).data[0]]) <= 1.0
        assert isinstance(in0, Variable) and in0.requires_grad
        assert len(in0.shape) == 4
        assert isinstance(in1, Variable)
        assert len(in1.shape) == 4
        return self.forward_pair(in0, in1)
